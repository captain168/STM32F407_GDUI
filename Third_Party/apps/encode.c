...
void jpeg_calc_value(jpeg_compress_info *cinfo) 
{
	...
	memset((void*)(cinfo->inbuf),0,cinfo->inbuf_size);//缓冲区清零	
	cinfo->next_line=0;//复状态值
}
///////////////////////////////////////////////////////////////////////////////////////////
u8  write_buf[512];
u16 wr_pt=0;
void jutl_write_byte(u8 c,char *f) 
{
	UINT WRBytes;
	...
	{
		f_write(&fileW,write_buf,wr_pt,&WRBytes);
		wr_pt=0;
	}
}
//开始压缩编码,写入头信息
