...
static int _DispGIFImage(IMAGE_DESCRIPTOR * pDescriptor, int x0, int y0, int Transparency, int Disposal)
{
  	...
  	for (YCnt = 0, YPos = y0, Pass = 0; YCnt < Height; YCnt++)
	{
    	...
    	for (XPos = x0; XPos <= XEnd; XPos++)
		{
	      	...
	      	else
		  	{
	        	if (Cnt)
				{
					...
					{
			            LCD_Line(XPos - Cnt - 1, YPos, XPos - 1, YPos, *(pTrans + OldIndex));
	          		}
					else if (Disposal == 2)
					{
			            LCD_Line(XPos - Cnt - 1, YPos, XPos - 1, YPos, BkColorIndex);
	          		}
	          		Cnt = 0;
	        	}
				else if (OldIndex >= 0)
				{
		            if (OldIndex != Transparency)LCD_SetPoint(XPos - 1, YPos, *(pTrans + OldIndex));
					else if (Disposal == 2)LCD_SetPoint(XPos - 1, YPos, BkColorIndex);
	          	}
	      	}
	      	OldIndex = Index;
	    }
	    if ((OldIndex != Transparency) || (Disposal == 2))
		{
	      	...
	      	if (Cnt)
			{
		        LCD_Line(XPos - Cnt - 1, YPos, XPos - 1, YPos, ColorIndex);
	      	}
			else LCD_SetPoint(XEnd, YPos, ColorIndex);
	    }
	    if (Interlace)
		{
...
{
	if(pDescriptor->YPos>pInfo->yPos)LCD_Fill(x0+pInfo->xPos,y0+pInfo->yPos,x0+pInfo->xPos+pInfo->xSize-1,y0+pDescriptor->YPos-1,BACKCOLOR);
	if(pDescriptor->XPos>pInfo->xPos)LCD_Fill(x0+pInfo->xPos,y0+pInfo->yPos,x0+pDescriptor->XPos-1,y0+pInfo->yPos+pInfo->ySize-1,BACKCOLOR);
	if((pDescriptor->YPos+pDescriptor->YSize)<(pInfo->yPos+pInfo->ySize))LCD_Fill(x0+pInfo->xPos,y0+pDescriptor->YPos+pDescriptor->YSize-1,x0+pInfo->xPos+pInfo->xSize-1,y0+pInfo->yPos+pInfo->ySize-1,BACKCOLOR);
	if((pDescriptor->XPos+pDescriptor->XSize)<(pInfo->xPos+pInfo->xSize))LCD_Fill(x0+pDescriptor->XPos+pDescriptor->XSize-1,y0+pInfo->yPos,x0+pInfo->xPos+pInfo->xSize-1,y0+pInfo->yPos+pInfo->ySize-1,BACKCOLOR);
}
...
void ShowGif(const void * Pgif, u32 NumBytes , int x0, int y0,u8 speed)
{
    static u16 i = 0;
